<?php
    ...
    $connFun = new connFun;
    $res = $connFun->query("SELECT * FROM answer where id = ".$pro_id);
    $num = mysql_num_rows($res);
    $pro_arr = array();
    if (mysql_num_rows($res) > 0) {
      // 输出数据
      while($row = mysql_fetch_assoc($res)) {
            $arr = array('id' => $row["id"],"answer"=>$row["answer"],"time"=>$row["time"]);
...
<form class="layui-form"> <!-- 提示：如果你不想用form，你可以换成div等任何一个普通元素 -->
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-block">
      <input type="text" id="answer" name="" placeholder="请输入" autocomplete="off" class="layui-input" value="<?php echo $pro_arr[0][answer]; ?>">
    </div>
  </div>
...
function modify(){
  ...
  btn: ['点错了', '确定'] //可以无限个按钮
    ...
    layer.close(layer.index);
     ...
     $.post('answer_modify_manager.php',{'id':id,'answer':answer,'time':time},function(res){
...
function delect(){
  ...
  btn: ['点错了', '确定'] //可以无限个按钮
    ...
    layer.close(layer.index);
     ...
     $.post('answer_delete.php',{'id':id},function(res){
               if (res.code>0) {
                  layer.msg(res.msg);
                  layer.alert(res.msg,{icon:2});
               }else{
